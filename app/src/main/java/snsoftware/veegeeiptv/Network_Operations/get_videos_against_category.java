...
import snsoftware.veegeeiptv.Adapters.movie_list_adapter;
...
public class get_videos_against_category {
    ...
    List<Movie> channelsList;
    String category_id;
    public get_videos_against_category(Context context,ListView channel_list, String username, String password,String category_id) {
        ...
        link.append(password);
        link.append("&action=get_vod_streams&category_id=");
    ...
    public void Call_Api(){
                ...
                link.toString(), new Response.Listener<String>()
        {
            ...
            public void onResponse(String response) {
                ...
                channelsList=new Gson().fromJson(response,new TypeToken<List<Movie>>(){}.getType());
                if(channelsList!=null&&channelsList.size()>0){
                    ...
                    link.append(".");
                    link.append(channelsList.get(0).getContainer_extension());
                    LocalBroadcastManager.getInstance(context).sendBroadcast(new Intent("first_vod_stream").putExtra("first_vod_stream",link.toString()));
                    channel_list.setAdapter(new movie_list_adapter(channelsList,context));

                }else{
                    Toast.makeText(context,"Movies Not found",Toast.LENGTH_LONG).show();
                }
                }
